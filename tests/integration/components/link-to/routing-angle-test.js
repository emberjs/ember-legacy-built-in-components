...
moduleFor(
  ...
  class extends ApplicationTestCase {
    ...
    async ['@test Using <LinkTo> inside a routable engine link within the engine'](
      assert
    ) {
      this.add(
        'engine:routable',
      ...
      assert.equal(
        this.$('#engine-link.active').length,
        1,
...
moduleFor(
  ...
  class extends ApplicationTestCase {
    ...
    async [`@test it does not call preventDefault if 'target' attribute is provided`](
      assert
    ) {
      this.addTemplate(
        'index',
    ...
    async [`@test [DEPRECATED] The <LinkTo /> component can use dynamic params`](
      assert
    ) {
      this.router.map(function () {
