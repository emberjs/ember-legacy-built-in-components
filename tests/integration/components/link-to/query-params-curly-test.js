...
moduleFor(
  ...
  class extends ApplicationTestCase {
    ...
    async ['@test [DEPRECATED] `query-params` can be used outside of `{{link-to}}'](
      assert
    ) {
      if (!DEBUG) {
        assert.expect(0);
...
moduleFor(
  ...
  class extends ApplicationTestCase {
    ...
    async ['@test it applies activeClass when query params are not changed'](
      ...
      this.shouldBeActive(
        assert,
        '#same-search-same-sort-child-and-parent > a'
      );
      this.shouldNotBeActive(
        assert,
        '#same-search-different-sort-child-and-parent > a'
      );
      this.shouldNotBeActive(
        assert,
        '#change-search-same-sort-child-and-parent > a'
      );
    }
